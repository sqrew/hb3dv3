...
impl CollisionCompute {
    ...
    pub fn read_collision_results_sync(&self, device: &wgpu::Device, queue: &wgpu::Queue) -> Result<Vec<CollisionPair>, Box<dyn std::error::Error>> {
        ...
        let pairs_size = (collision_count.min(MAX_COLLISIONS) as usize * std::mem::size_of::<CollisionPair>()) as u64;
        encoder.copy_buffer_to_buffer(
            &self.collision_pairs_buffer,
            0,
            &self.collision_pairs_staging,
