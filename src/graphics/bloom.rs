...
impl BloomRenderer {
    pub fn new(
        ...
        // Create textures
        let scene_texture = Self::create_render_texture(&device, width, height, surface_format, "Scene");
        let scene_texture_view = scene_texture.create_view(&wgpu::TextureViewDescriptor::default());
        
        let bright_texture = Self::create_render_texture(&device, width / 2, height / 2, surface_format, "Bright");
        let bright_texture_view = bright_texture.create_view(&wgpu::TextureViewDescriptor::default());
        
        let blur_texture_1 = Self::create_render_texture(&device, width / 2, height / 2, surface_format, "Blur1");
        let blur_texture_1_view = blur_texture_1.create_view(&wgpu::TextureViewDescriptor::default());
        
        let blur_texture_2 = Self::create_render_texture(&device, width / 2, height / 2, surface_format, "Blur2");
        let blur_texture_2_view = blur_texture_2.create_view(&wgpu::TextureViewDescriptor::default());
        ...
        let composition_uniforms = CompositionUniforms {
            bloom_intensity: 0.6,  // Higher intensity for neon effect
            bloom_radius: 1.0,
            exposure: 1.1,         // Slight exposure boost for neon brightness
            _padding: 0.0,
        };
        
        let composition_uniform_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        ...
        let composition_settings_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
