...
impl PrimitiveLineData {
    ...
    pub fn generate_instances_with_rotation(
        ...
        if self.line_segments.len() > 50 {
            ...
            let parallel_instances: Vec<LineInstance> = self
                .line_segments
                .par_iter()
                .map(|(start_local, end_local)| {
                    // Apply full matrix transformation to each point - parallelized
                    let start_world = transform_matrix.transform_point(&nalgebra::Point3::new(
                        start_local[0],
                        start_local[1],
                        start_local[2],
                    ));
                    let end_world = transform_matrix.transform_point(&nalgebra::Point3::new(
                        end_local[0],
                        end_local[1],
                        end_local[2],
                    ));
...
impl PrimitiveCache {
    ...
    pub fn generate_line_instances(
        ...
        self.get_primitive_data(primitive_type).generate_instances(
            world_position,
            scale,
            color,
            thickness,
    ...
    fn generate_sphere_data() -> PrimitiveLineData {
        ...
        for ring in 0..=SPHERE_RINGS {
            ...
            for segment in 0..SPHERE_SEGMENTS {
                ...
                let next_phi =
                    2.0 * std::f32::consts::PI * ((segment + 1) % SPHERE_SEGMENTS) as f32
    ...
    fn generate_cylinder_data() -> PrimitiveLineData {
        ...
        for i in 0..CYLINDER_SEGMENTS {
            ...
            let next_angle = 2.0 * std::f32::consts::PI * ((i + 1) % CYLINDER_SEGMENTS) as f32
                / (CYLINDER_SEGMENTS as f32);
    ...
    fn generate_tetrahedron_data() -> PrimitiveLineData {
        let vertices = [
            ...
            [
                -TETRAHEDRON_BASE_X_OFFSET,
                TETRAHEDRON_BASE_Y,
                TETRAHEDRON_BASE_FRONT,
            ], // 1: base front left
            [
                TETRAHEDRON_BASE_X_OFFSET,
                TETRAHEDRON_BASE_Y,
    ...
    fn generate_cone_data() -> PrimitiveLineData {
        ...
        for i in 0..CONE_SEGMENTS {
            ...
            let next_angle = 2.0 * std::f32::consts::PI * ((i + 1) % CONE_SEGMENTS) as f32
                / (CONE_SEGMENTS as f32);
    ...
    fn generate_torus_data() -> PrimitiveLineData {
        ...
        for i in 0..TORUS_MAJOR_SEGMENTS {
            ...
            let next_major_angle = ((i + 1) % TORUS_MAJOR_SEGMENTS) as f32
                / TORUS_MAJOR_SEGMENTS as f32
                * 2.0
                * std::f32::consts::PI;
            ...
            for j in 0..TORUS_MINOR_SEGMENTS {
                let minor_angle =
                    (j as f32 / TORUS_MINOR_SEGMENTS as f32) * 2.0 * std::f32::consts::PI;
                let next_minor_angle = ((j + 1) % TORUS_MINOR_SEGMENTS) as f32
                    / TORUS_MINOR_SEGMENTS as f32
                    * 2.0
                    * std::f32::consts::PI;
    ...
    fn generate_dodecahedron_data() -> PrimitiveLineData {
        ...
        let edges = [
            ...
            (0, 8),
            (0, 12),
            (0, 16), // vertex 0 connections
            (1, 9),
            (1, 12),
            (1, 18), // vertex 1 connections
            (2, 10),
            (2, 13),
            (2, 16), // vertex 2 connections
            (3, 11),
            (3, 13),
            (3, 18), // vertex 3 connections
            (4, 8),
            (4, 14),
            (4, 17), // vertex 4 connections
            (5, 9),
            (5, 14),
            (5, 19), // vertex 5 connections
            (6, 10),
            (6, 15),
            (6, 17), // vertex 6 connections
            (7, 11),
            (7, 15),
            (7, 19), // vertex 7 connections
    ...
    fn generate_capsule_data() -> PrimitiveLineData {
        ...
        for ring in 1..hemisphere_rings {
            ...
            for i in 0..segments {
                ...
                let (x2, z2) = (
                    next_angle.cos() * ring_radius,
                    next_angle.sin() * ring_radius,
                );

                // Top hemisphere rings
                line_segments.push((
                    [x1, cylinder_height + ring_y, z1],
                    [x2, cylinder_height + ring_y, z2],
                ));
        ...
        for i in 0..segments / 2 {
            // Only need half the meridians to avoid clutter
            let angle = 2.0 * std::f32::consts::PI * i as f32 / segments as f32;
            ...
            line_segments.push((
                [x, cylinder_height, z],
                [0.0, cylinder_height + radius, 0.0],
            ));
    ...
    fn generate_hemisphere_data() -> PrimitiveLineData {
        ...
        for ring in 0..=(SPHERE_RINGS / 2) {
            ...
            for segment in 0..SPHERE_SEGMENTS {
                ...
                if ring < SPHERE_RINGS / 2 {
                    let next_theta =
                        std::f32::consts::PI * ((ring + 1) as f32) / (SPHERE_RINGS as f32);
        ...
        for segment in 0..SPHERE_SEGMENTS {
            let phi = 2.0 * std::f32::consts::PI * (segment as f32) / (SPHERE_SEGMENTS as f32);
            let next_phi = 2.0 * std::f32::consts::PI * ((segment + 1) % SPHERE_SEGMENTS) as f32
                / (SPHERE_SEGMENTS as f32);
