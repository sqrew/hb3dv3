...
impl Scheduler {
    ...
    pub fn update_with_physics(
        ...
        let new_fractal_entities = self
            .bullets
            .update_with_targets(delta_time, &enemy_positions);

        // Register new fractal bullet entities with the entity manager
        for entity_id in new_fractal_entities {
            self.entity_manager.register_existing_entity(
                entity_id,
                crate::engine::entity::EntityType::PlayerBullet,
            );
        }
